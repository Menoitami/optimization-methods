...
#   coords - array of statistics
    ...
    while (norm(deltaX) >= tol) and (k < kmax):
        gk = df(x0)
        Gesse = H(x0, tol, df)
        inv_Gesse = np.linalg.inv(Gesse)

        sigmak = np.sqrt(gk.T @ inv_Gesse @ gk)
        if sigmak> 1/4:
            a = 1/(1+sigmak)
        else:
            a= 1

        xk = x0 - a* np.linalg.lstsq(-Gesse, -gk)[0] 
...
print(nestersearch(fR,dfR,[-2,-2], 1e-5 )[0:2])